...
function scene:initMap()
	...
	self.map = sti("res/tile/map2.lua", { "bump","light" })
...
function scene:switch()
	...
	gamestate.switch(gameState.level2, love.graphics.newImage(love.graphics.newScreenshot()),"zoom",cx,cy)
end

function scene:fade(screenshot,how)
	...
	elseif how == "zoom" then
		...
		self.draw = function(self)
			...
			zoom = zoom + self.globalScale*2* love.timer.getDelta()
			if zoom<4 then
				love.graphics.draw(screenshot,w()/2,0,0,zoom,zoom,w()/2,h()/2)
			else
				love.graphics.draw(self.canvas,w()/2,0,0,self.globalScale*(8-zoom),self.globalScale*(8-zoom),self.w/2,self.h/2)
			end
			self:drawLabels()
			self.menu:draw()
		end
